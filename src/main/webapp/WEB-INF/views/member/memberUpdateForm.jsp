...
<script type="text/javascript">
	$(document).ready(function(){
		var b_seq = '${member.b_seq}'; 
		var team_seq = '${member.t_seq}';
		...
		success: function(data) {
			for(var i=0 in data){
				if(data[i].t_seq == team_seq){
					$('#t_seq').append("<option value='"+data[i].t_seq+"' selected='selected'>"+data[i].t_name+"</option>");
				} else if(data[i].t_seq != team_seq) {
					$('#t_seq').append("<option value='"+data[i].t_seq+"'>"+data[i].t_name+"</option>");
				}
			}	
		}
	});
	...
	function chk() {
		...
		if(frm.m_name.value==""){
			...
			if(frm.m_password.value.length < 8){
	...
	function deleteFile(i) {
		...
		if(cf) {
			$.ajax({
				url : "${path}/member/deleteFile",
				type : 'post',
				data : {'af_seq' : i},
				success : function (data) {
					$('#file_'+i).children('td').remove();
					$('#file_'+i).append("<td align='center' colspan='4'>삭제 되었습니다.</td>")
				}
			});
		}
	}
	function reSetPass(i) {
		var cf = confirm("정말로 비밀번호를 초기화 하시겠습니까?");
		if(cf) {
			$.ajax({
				url : "${path}/member/reSetPass",
				type: 'post',
				data : {'m_id' : i},
				success : function (data) {
...
<body>
	<div class="container" align="center">
		...
		<div align="left">
			<a class="btn btn-success btn-lg" href="#" onclick="history.go(-1);">뒤로가기</a>
		</div>
		...
		<form action="${path }/member/memberUpdate" method="post" enctype="multipart/form-data" name="frm">
			<table class="table-bordered mt-3">
				...
				<tr>
					...
					<td align="center"><input type="text" class="form-control input-defualt" name="m_name" value="${member.m_name }" ></td>
					<th>비밀번호 초기화</th>
					<td align="center">
						<input type="button" class="btn btn-danger btn-lg" onclick="reSetPass('${member.m_id}')" value="초기화">
					</td>
				</tr>
				<tr>
					<th>주민번호<span class="required">*</span></th>
					<td align="center">
						<input type="text" class="form-control input-defualt" name="m_ssnum" onkeyup="inputSSNumber(this)" maxlength="14" value="${member.m_ssnum }" placeholder="숫자만 입력하세요" >
					</td>
					<th>이메일<span class="required">*</span></th>
					<td align="center">
						<input type="email" class="form-control input-defualt" name="m_email" value="${member.m_email }">
					</td>
				</tr>
				<tr>
					<th>전화번호<span class="required">*</span></th>
					<td align="center">
						<input type="text" class="form-control input-defualt" name="m_tel" value="${member.m_tel }" onkeyup="inputPhoneNumber(this);" maxlength="13" placeholder="숫자만 입력하세요" >
					</td>
					<th>입사일<span class="required">*</span></th>
					<td align="center">
						<input type="date" class="form-control input-defualt" name="m_hiredate" value="${member.m_hiredate }">
					</td>
				</tr>
				<tr>
					<th rowspan="4">주소<span class="required">*</span></th>
					<td colspan="3">
						<input type="text" class="form-control input-post" name="m_postcode" id="postcode" value="${member.m_postcode }" placeholder="우편주소" >
						<input type="button" class="btn btn-primary btn-sm" onclick="execDaumPostcode()" value="주소검색">
					</td>
				</tr>
				<tr>
					<td colspan="3">
						<input type="text" class="form-control input-defualt" name="m_addr" id="address" value="${member.m_addr }" placeholder="주소" >
					</td>
				</tr>
				<tr>
					<td colspan="3">
						<input type="text" class="form-control input-defualt" name="m_detailaddr" id="detailAddress" value="${member.m_detailaddr }" placeholder="상세주소" >
					</td>
				</tr>
				<tr>
					<td colspan="3">
						<input type="text" class="form-control input-defualt" name="m_extraaddr" id="extraAddress" value="${member.m_extraaddr }" placeholder="참고항목">
					</td>
				</tr>
				<tr>
					<th>도입자</th>
					<td align="center">
						<input name="m_mgr" id="mgr" type="text" class="form-control input-defualt" style="width:60%; display: inline-block;" placeholder="도입자 아이디" value="${member.m_mgr }" readonly="readonly">
							...
							<c:if test="${not empty roleList }">
								<c:forEach var="role" items="${roleList }">
									<c:if test="${role.r_code != 'c2' }">
										<c:if test="${member.r_code == role.r_code }">
											<option value="${member.r_code }" selected="selected">${role.r_name }</option>
										</c:if>
										<c:if test="${member.r_code != role.r_code }">
					...
					<td align="center">
						...
						<c:if test="${not empty branchList }">
							<c:forEach var="branch" items="${branchList }">
								<c:if test="${branch.b_seq == member.b_seq }">
									<option value="${branch.b_seq }" selected="selected">${branch.b_name }</option>
								</c:if>
								<c:if test="${branch.b_seq != member.b_seq }">
				...
				<c:if test="${not empty fileList }">
					<c:forEach var="file" items="${fileList }">
						<tr id="file_${file.af_seq }">
							<td align="center">${fileCount }<c:set var="fileCount" value="${fileCount-1 }"></c:set></td>
							<td colspan="2" align="center">
								${file.af_name }.${file.af_type } 
								<span style="float: right; margin-right: 10px;">
									<a href="#" onclick="deleteFile('${file.af_seq}')">삭제</a>
								</span>
							</td>
							<td align="center">
								${file.af_date }
			...
			<input type="submit" class="btn btn-warning btn-lg mt-3" onclick="return chk();" value="수정하기">
		</form>
	</div>
</body>
</html>